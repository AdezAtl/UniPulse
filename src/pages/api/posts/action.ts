...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (action === 'delete') {
    ...
    if (isAdmin && !isAuthor) await addAdminLog(userId, 'delete_post', 'post', postId);
    return ok();
  }
  if (action === 'restore') {
    await updatePost(postId, { is_deleted: 0 });
    if (isAdmin) await addAdminLog(userId, 'restore_post', 'post', postId);
    return ok();
  }
  if (action === 'flag') {
    await updatePost(postId, { is_flagged: 1 });
    if (isAdmin) await addAdminLog(userId, 'flag_post', 'post', postId);
    return ok();
  }
  if (action === 'unflag') {
    await updatePost(postId, { is_flagged: 0 });
    if (isAdmin) await addAdminLog(userId, 'unflag_post', 'post', postId);
    return ok();
  }
  if (action === 'edit') {
    ...
    if (isAdmin && !isAuthor) await addAdminLog(userId, 'edit_post', 'post', postId, { new_content: trimmed });
    return ok();
  }
