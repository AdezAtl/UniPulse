...
const DATA_DIR = process.env.DATABASE_DIR || join(__dirname, '../../data');
...
export function getFeedPosts(limit?: number | null): PostWithMeta[] {
  ...
  return (db.prepare(`
    ...
    LIMIT ?
  `).all(actualLimit) as any[]).map(normalisePost);
}
...
export function getAllPostsAdmin(limit?: number | null): PostWithMeta[] {
...
export function getAdminLogs(limit?: number | null): AdminLog[] {
  const actualLimit = typeof limit === 'number' && limit > 0 ? limit : 60;
  return (db.prepare(`
    ...
    LIMIT ?
  `).all(actualLimit) as any[]).map(row => ({
    ...row,
...
export function getUpdates(userId: string) {
  const unreadNotifications = getUnreadNotificationCount(userId);
  
  const msgRow = db.prepare(`SELECT COUNT(*) as c FROM messages WHERE receiver_id = ? AND is_read = 0`).get(userId) as any;
